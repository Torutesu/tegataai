...
- [x] waitlist のバックエンド（`POST /v1/waitlist`。実ブラウザで EN/JA 両方の登録を確認済み）
- [ ] **公開**（LP / 診断ツール / spec repo / 動画）← 商標 OK。**残りはオーナー作業のみ**
      （npm scope / DNS / ホスティングの契約）。設定・検証は済んでいる:
      `pnpm check:deployed` が公開直後の確認を1コマンドで行い、CI がコンテナを
      ビルドして起動・再起動まで通す
...
| # | 成果物 | 状態 |
|---|---|---|
| 1 | 商標調査・ドメイン/ハンドル確保 | **オーナー承認済（2026-08-21）**。依頼書は `docs/gtm/trademark-brief.md` |
