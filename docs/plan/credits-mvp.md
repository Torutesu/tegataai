...
## 12.2 セキュリティレビュー（2026-08-21）

公開前レビューで**重大な脆弱性を1件**発見・修正した。詳細は `docs/plan/security-review.md`。

- **S-01（重大）**: Stripe webhook で、署名ヘッダを省略すると検証がスキップされていた。
  テナント ID を知る誰もが偽の `invoice.paid` でクレジットを無制限に発行できた。
  → 検証を必須化（D-34）
- **S-02（中）**: 署名を再シリアライズしたボディに対して検証していた → 生バイトに対して検証
- **S-03（低）**: `updateTenant` の動的 SQL 列名 → 許可列を固定

テナント分離・秘密の非漏洩・認証をすり抜ける経路の不在は、いずれもテストで確認済み。

## 13.1 MVP に残る既知の限界（隠さない）

0. **レート制限は単一ノードのメモリ内**。認証後にしか効かないので、認証前の物量には
   前段（CDN / LB）が要る
